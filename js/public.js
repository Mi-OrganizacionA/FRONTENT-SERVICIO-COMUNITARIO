...
(function () {
  ...
  function renderStats(stats, totalProyectos) {
    ...
    const map = {
      ...
      't5Asambleas': 24,
      't5UBCH': 3,
      't6Elec': stats.servicios ? stats.servicios.electricidad : 0,
      't6Agua': stats.servicios ? stats.servicios.agua : 0,
      't6Gas': stats.servicios ? stats.servicios.gas : 0,
      't7Alianzas': 3,
      't7Inter': 5,
      't7Jornadas': 14,
  ...
  modalDetalleOverlay?.addEventListener('click', (e) => {
    if (e.target === modalDetalleOverlay) {
      window.cerrarModales();
    }
  });

  document.getElementById('modalExploradorClose')?.addEventListener('click', window.cerrarModales);
  modalExploradorOverlay?.addEventListener('click', (e) => {
    if (e.target === modalExploradorOverlay) {
      window.cerrarModales();
    }
  });
